#########################################################################
# function from https://github.com/noaa-psd/pyCOARE
# it needs meteo.py and util.py (download from the same link)
#########################################################################
"""
...
#########################################################################
# function that applies COARE3.5 bulk formulas to wrfout file
# it reads directly wrfout as input
#########################################################################
"""
This function returns:
- friction velocity that includes gustiness (m/s)
- wind stress (N/m^2)
- sensible heat flux into ocean (W/m^2)
- latent heat flux into ocean (W/m^2)
...
def coare4wrf(wrf_file):
    # input constants
    zu = 10     # wind sensor height (m)
    zt = 2      # bulk temperature sensor height (m)
    zq = 2      # RH sensor height (m)

    # Read the output files.
    ds_atm = xr.open_dataset(wrf_file)
            
    # Select the atmospheric variables of interest.
    lon_atm = ds_atm['XLONG'][0].values
    lat_atm = ds_atm['XLAT'][0].values
    sst = ds_atm['SST'][0].values
    t2 = ds_atm['T2'][0].values
    u10 = ds_atm['U10'][0].values
    v10 = ds_atm['V10'][0].values
    U = np.sqrt(u10**2+v10**2)
    PSFC = ds_atm['PSFC'][0].values
    es = 611*np.exp(5.42*10**3*(1/273-1/sst))
    ws = 0.622*es/(PSFC - es)
    q2 = ds_atm['Q2'][0].values
    rh = 100*q2/ws
    SWDOWN = ds_atm['SWDOWN'][0].values
    GLW = ds_atm['GLW'][0].values
    PBLH = ds_atm['PBLH'][0].values
    ...
    seamask_nan = seamask
    seamask_nan[seamask_nan==0] = np.nan
    ...
    U = U*seamask_nan
    rh = rh*seamask_nan
    PSFC = PSFC*seamask_nan
    SWDOWN = SWDOWN*seamask_nan
    GLW = GLW*seamask_nan
    PBLH = PBLH*seamask_nan

    # config variables for function coare
    sst = sst - 273.15
    t2 = t2 - 273.15
    PSFC = PSFC/100
    # reshape to 1d array
    ...
    rh = rh.reshape(-1)
    PSFC = PSFC.reshape(-1)
    SWDOWN = SWDOWN.reshape(-1)
    GLW = GLW.reshape(-1)
    PBLH = PBLH.reshape(-1)
    lat = lat_atm.reshape(-1)

    # run coare function
    A = coare35vn(U, t2, rh, sst, PSFC, SWDOWN, GLW, zu, zt, zq, lat, PBLH, jcool=1) 

    # select output
    rows=lat_atm.shape[0]
    cols=lat_atm.shape[1]
    usr = A[:,0].reshape(rows,cols)     # friction velocity that includes gustiness (m/s)
    tau = A[:,1].reshape(rows,cols)     # wind stress (N/m^2)
    hsb = A[:,2].reshape(rows,cols)     # sensible heat flux into ocean (W/m^2)
    hlb = A[:,3].reshape(rows,cols)     # latent heat flux into ocean (W/m^2)
    Cd = A[:,9].reshape(rows,cols)      # wind stress transfer (drag) coefficient at height zu
    Ch = A[:,10].reshape(rows,cols)     # sensible heat transfer coefficient (Stanton number) at ht zu
    Ce = A[:,11].reshape(rows,cols)     # latent heat transfer coefficient (Dalton number) at ht zq
    L = A[:,12].reshape(rows,cols)      # Obukhov length scale (m)

    return usr,tau,hsb,hlb,Cd,Ch,Ce,L


#########################################################################
# function that applies COARE3.5 bulk formulas 
# it needs variables U, t2, rh, sst, PSFC, SWDOWN, GLW, lat, PBLH
#########################################################################
"""
This function returns:
- friction velocity that includes gustiness (m/s)
...
import warnings
warnings.filterwarnings("ignore")
def coare_var(U, t2, rh, sst, PSFC, SWDOWN, GLW, lat, PBLH, jcool):
    # input constants
    zu = 10     # wind sensor height (m)
    zt = 2      # bulk temperature sensor height (m)
    zq = 2      # RH sensor height (m)
    ...
    # run coare function
    if jcool == 0:
        A = coare35vn(U, t2, rh, sst, PSFC, SWDOWN, GLW, zu, zt, zq, lat, PBLH, jcool=0) 
    elif jcool == 1:
        A = coare35vn(U, t2, rh, sst, PSFC, SWDOWN, GLW, zu, zt, zq, lat, PBLH, jcool=1)
