...
def neggers_et_al_2006_stevens_et_al_2002(t,y,SST,D,q_free,th_free,ps,f,U_free,V_free, dict_ext):
    """
...
#    w_star = (g*y[0]*thv_flux_s/th_s)**(1/3) # m/s, Deardorff convective velocity scale. 1st try    
    ...
    dict_ext['SHF_ext'].append(th_flux_s*cpd*rhos)       # SHF_ext
...
def neggers_et_al_2006_stevens_et_al_2002_fracUstar(t,y,SST,D,q_free,th_free,ps,f,U_free,V_free, dict_ext, frac_Ustar, thermSubsi):
    """
    Subscript s denotes surface values, b stands for buoyancy, which is associated with the virtual potential
    temperature (theta_v), th indicates the liquid water potential temperature (theta_l) and q the total specific
    humidity (q_t).
    ...
    y[4] is V, the meridional bulk wind component
    V_mag is the magnitude of the bulk wind = np.sqrt(y[3]**2+y[4]**2)
    thermSubsi : if 1: includes subsidence in thermodynamics; if 0 : leaves the bulk model as it is
    """
    # Define some constants.
    delta_z = 100.0                       # [m]
    Cqs = 0.0012                          # [1], surface exchange coefficient for q
...
#    w_star = (g*y[0]*thv_flux_s/th_s)**(1/3) # m/s, Deardorff convective velocity scale. 1st try    
    ...
    # Entrainment for the horizontal momentum.
    u_star = frac_Ustar*V_mag*np.sqrt(CD) # [m/s], Friction velocity, assuming air density equal to 1... We can correct this!
    L = - u_star**3/(0.4*b_flux_s)# [m], Monin-Obukhov length
    ...
    ### Define the equations to be solved.
    dh_dt = E - D*y[0] - M 
    dq_dt = (q_flux_s + (E-D*y[0]*thermSubsi)*delta_q)/y[0] #+ F_advq
    dth_dt = (th_flux_s + (E-D*y[0]*thermSubsi)*delta_th)/y[0] + F_advth + F_rad
    dU_dt = f*(y[4]-V_free)-y[3]*(CD*V_mag+we_dyn)/y[0]+U_free*we_dyn/y[0]
    dV_dt = -f*(y[3]-U_free)-y[4]*(CD*V_mag+we_dyn)/y[0]+V_free*we_dyn/y[0]
...
##### subsidence also has a role in moving moisture and temperature 
##### from the upper troposphere into the MABL

def neggers_et_al_2006_stevens_et_al_2002_ThermSubsidence(t,y,SST,D,q_free,th_free,ps,f,U_free,V_free, dict_ext):
    """
    Subscript s denotes surface values, b stands for buoyancy, which is associated with the virtual potential
    temperature (theta_v), th indicates the liquid water potential temperature (theta_l) and q the total specific
    humidity (q_t).
...
#    w_star = (g*y[0]*thv_flux_s/th_s)**(1/3) # m/s, Deardorff convective velocity scale. 1st try    
    ...
    ### Define the equations to be solved.
    dh_dt = E - D*y[0] - M 
    dq_dt = (q_flux_s + (E-D*y[0])*delta_q)/y[0] + F_advq
    dth_dt = (th_flux_s + (E-D*y[0])*delta_th)/y[0] + F_advth + F_rad
    dU_dt = f*(y[4]-V_free)-y[3]*(CD*V_mag+we_dyn)/y[0]+U_free*we_dyn/y[0]
    dV_dt = -f*(y[3]-U_free)-y[4]*(CD*V_mag+we_dyn)/y[0]+V_free*we_dyn/y[0]
...
def fullMod_fracUstar_dynamicFrad(t,y,SST,D,q_free,th_free,ps,f,U_free,V_free,we, dict_ext, frac_Ustar):
    """
    Subscript s denotes surface values, b stands for buoyancy, which is associated with the virtual potential
    temperature (theta_v), th indicates the liquid water potential temperature (theta_l) and q the total specific
    humidity (q_t).
...
#    w_star = (g*y[0]*thv_flux_s/th_s)**(1/3) # m/s, Deardorff convective velocity scale. 1st try    
    ...
    ##########  LET'S TRY TO FORCE AN INTERACTIVE RADIATIVE COOLING  #########
    SB_const = 5.67*1e-8
    F_rad = -(1-area_c)*SB_const*(SST**4)  + area_c*(-2/86400)
...
def neggers_stevens_FracE(t,y,SST,D,q_free,th_free,ps,f,U_free,V_free,we, dict_ext,E_frac):
    """
    Subscript s denotes surface values, b stands for buoyancy, which is associated with the virtual potential
    temperature (theta_v), th indicates the liquid water potential temperature (theta_l) and q the total specific
    humidity (q_t).
...
def neggers_stevens_boxModel(t,y,SST,D,q_free,th_free,ps,f,U_free,V_free,we, dict_ext, E_frac=1):
    """
    Subscript s denotes surface values, b stands for buoyancy, which is associated with the virtual potential
    temperature (theta_v), th indicates the liquid water potential temperature (theta_l) and q the total specific
    humidity (q_t).
